...
func initDb(cfg config.AppConfig) (connection *gorm.DB, err error) {
	...
	if cfg.DBConfig.DbDriver == "postgres" {
		...
		sqltrace.Register("pgx", &stdlib.Driver{}, sqltrace.WithServiceName("goten-db"))
		...
		connection, err = gormtrace.Open(postgres.New(postgres.Config{Conn: sqlDb}), &gorm.Config{}, gormtrace.WithServiceName(`goten-db-gorm`))
		if err != nil {
